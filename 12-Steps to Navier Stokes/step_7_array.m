...
for t = 1:nt
    un = u;
    u(2:nx-1,2:ny-1) = un(2:nx-1,2:ny-1)+vis.*(dt/dx^2).*(un(3:nx,2:ny-1)-2.*un(2:nx-1,2:ny-1)+un(1:nx-2,2:ny-1))...
                    +vis.*(dt/dy^2).*(un(2:nx-1,3:ny)-2.*un(2:nx-1,2:ny-1)+un(2:nx-1,1:ny-2));
    pause(0.05)
    surf(X,Y,u(:,:))
    axis([0 2 0 2 1 2.5])
end
